...
In the case of IROS 2003, which took place at a Las Vegas casino, the task was to create a robot that could �gamble� by placing plastic tokens into a toy slot machine, pulling a lever and catching the chips as they popped out.  One team was able to create a solution with PolyBot G1V4 system with 14 modules (14 DoF). The participants had 6 hours to build on design primitives they created the previous day (snake-like mobility, and arm like structures) that would solve the task - which they almost did - they did not catch any chips.
Using our framework and library, complex designs (such as the Walkbot or 18-module snake) can be created and programmed in under an hour and just a few minutes if the primitives have already been created. These are of similar complexity to the IROS system except does not include the hardware component nor hardware debugging. The other competitions listed had similar tasks, time and modular robot resources.  While this is not an apples to apples comparison, having the ability to build and test configurations in simulation, in this case, would be orders of magnitude faster.
