...
system is integrated with a full dynamic simulator, and provides tools to \abj{automatically}
...
decades \TODO{citations}.  These systems distinguish themselves \abj{from conventional robotic systems} in their
...
for programming them \cite{}.  Over 800 papers, a book \cite{stoy2010book}, and a survey \cite{yim2007modular}
have been written on the subject. }

This \dbj{additional }versatility places an additional burden on the user, because
...
impractical. If the user is free to create any new design \dbj{he/she pleases }to
...
ROS\cite{Quigley2009} which provides IPC and standard libraries for common robot tasks. 

The challenges of \cbj{software design}{designing controls} are different in modular robotics than
...
behaviors (\cbj{\textit{i.e.}}{\textit{e.g.}} take a step with a leg) with new morphologies that share some of the
structure of old morphologies.

Our solution to this problem is to let the modularity of our hardware \cbj{determine}{guide} the
modularity of our \cbj{software}{designs of control}. We create new modular robot designs by combining existing sub-designs,
...
associated libraries of \dbj{software }behaviors, so that when new designs are created
...
sub-designs. We introduce \abj{a} new \cbj{scripting language}{control composition formalism} with a series-parallel
...
\cbj{design specification by a novice user}{novice users' design specifications}.  Users create new designs by composing
...
designed and built once, and then programmed many times.  In the case of a \cbj{MRS}{modular robot system}, the system can be reconfigured to meet new tasks,
...
low-level behaviors (\cbj{i.e.}{e.g.} motor drivers appropriate for motors\abj{)}, we do so for
...
robots \cite{zhang2003phase}. In this paper, we present a \cbj{novel scripting language}{computer aided design that enables novice}
...
with each \(x_i\) corresponding to one of the \(d\) degrees of freedom \abj{(DoF)} of the module.
...
\abj{Figure~\ref{fig:smores} shows a module with four DoF and four attachment points}
\end{definition}
...
X, B)$, where:
\begin{itemize}
\item $C$ is a set of \abj{sub-}configurations, $C=\{\mathcal{C}_{1}, \mathcal{C}_{2}, ..., \mathcal{C}_{q}\}$.
\item $\gamma: C \rightarrow M$ is a function \cbj{that, when given}{mapping} a configuration\cbj{, returns}{to} all modules of that configuration.
...
\item \(B\) is a set of \textit{behaviors} \abj{(Definition~\ref{def:behavior})} associated with the configuration.
\end{itemize}

Figure~\ref{fig:smores_conf_photo} shows a photo of a configuration composed
of three \cbj{SMORES modules}{modules each with four attachment points}.  Figure~\ref{fig:smores_conf_graph} shows its
...
controller setpoint for that \cbj{degree of freedom}{DoF}, \(\xi\) specifies an interrupt
...
Here, \(\theta_0\) denotes the currently-sensed value of \cbj{theta}{$\theta$} at the beginning of
the TURN behavior.
...
\dbj{(DoF) }- three continuously rotating faces we call {\em turntables} and one
...
by composing CHAIN3\dbj{'s} end-to-end; Figure \ref{fig:snake18} shows one with 18 modules.
