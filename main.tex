...
\TODO{Mention applications (SAR etc).}
...
\TODO{Move this SMORES stuff to come later} For a single SMORES module, there are four degree of freedom as shown in Figure~\ref{fig:SmoresRobot}, and there are four nodes to connect to other modules, i.e. $N=\{$front, left, right, back$\}$ as shown in Figure~\ref{fig:smores}.
...
configurations by the topology of their component modules, we define them recursively,
...
of three SMORES modules \TODO{technically we haven't introduced SMORES\ yet...}. Figure~\ref{fig:smores_conf_graph} shows its
...
of other modules, we may continue composing displacements in the manner above until
we reach the desired module.
...
\paragraph{Stability Checking}
 Gravitational stability is checked by computing the location of the center of mass of the configuration. For the given configuration $\mathcal{C}$, the position of the center of mass is
\begin{equation*}
...
Figure~\ref{fig:design} demonstrates the design low when a user is designing a configuration an its behaviors. We present the start-to-end user perspective in designing a complicated configuration called Walkbot. Consider a basic configuration formed by three modules in a line. We can form a ``body'' configuration by connecting two of the basic configurations as shown in Figure~\ref{fig:walkbot1}. With four more basic configurations and attach sides of those basic configurations to sides of the ``body'' configuration with certain angle offset, we can build a complex configuration, Walkbot, with four legs, as shown in Figure~\ref{fig:walkbot2}. Notice that by connecting multiple  basic configurations together, we can design a complex configuration without building with individual modules.
