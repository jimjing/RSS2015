...
and attach to other modules.  Formally, we define a  module  as $\mathcal{M}=(P_{/\mathcal{R}}, O_{/\mathcal{R}}, X, A)$, where:
\begin{itemize}
...
\item $A=\{a_1, a_2, ..., a_k\}$ is the set of attachment points where the module can connect to other modules.
\end{itemize}
...
Formally, we define a configuration as $\mathcal{C}=(C_, \gamma, M, \mathcal{M}_b, E, \delta,
X, B)$, where:
\begin{itemize}
...
\item $\delta: E \rightarrow \mathbb{R}$ is a labeling function over a connection. Given a connection, $\delta$ returns the fixed
angular
 offset in degrees at the attachment point of two connected modules.
\item \(X = \displaystyle\bigcup_{m_i \in M} X_i \) is the \textit{state} of the configuration.
\item \(B\) is a set of behaviors associated with the configuration.
\end{itemize}
...
zero over time.
\end{definition}
\begin{definition}[Behaviors Conflict]
