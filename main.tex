...
composing behaviors from a library in a nested series/parallel structure. The
...
\begin{itemize}
\item There is no collision between modules at all time during the execution of the behavior.
...
For the library to be most effective, the set of configurations and behaviors available
 ...
 we see that a large  and diverse set of second- and third-order configurations can
...
\section{Results}
In the past, designing configurations and behaviors to address new tasks has
required time on the order of one to dozens of hours \cite{sastra2011using}.  To
evaluate the rapid creation of solutions for robotic tasks using modular robot
systems, we can look to the modular robot competitions held at large
conferences IROS 2003, ICRA 2008, ICRA 2010, ICRA 2011 and ICRA 2012. In each
case participants used modular robot systems to solve problems with little a
priori knowledge of the task (except that the task would likely require remote
operation/manipulation of objects).

In the case of IROS 2003, which took place at a Las Vegas casino, the task was
to create a robot that could “gamble” by placing plastic tokens into a
toy slot machine, pulling a lever and catching the chips as they popped out.
One participant was able to take the PolyBot G1V4 system with 14 modules (14
DoF) and create. The participants had 6 hours to build on design primitives
they created the previous day (snake-like mobility, and arm like structures)
that would solve the task - which they almost did - they did not catch any
chips.

Using our framework and library, complex designs (such as the Walkbot or
18-module snake) can be created and programmed in under an hour and just a few
minutes if the primitives have already been created. These are of similar
complexity to the IROS system except does not include the hardware component
nor hardware debugging. The other competitions listed had similar tasks, time
and modular robot resources.  While this is not an apples to apples comparison,
having the ability to build and test configurations in simulation, in this
case, would be orders of magnitude faster. 
