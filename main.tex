...
\abj{Checking collisions between objects with complex structure in space is not a trivial problem \TODO{cite}. In this work, we simplify this problem by treating each module as a sphere with a fixed radius. A collision between two modules can be detected by comparing the distance between the centroid of spheres and the sum of radius. Existing algorithms on collision detection for complex structures \TODO{cite} can be used for higher precision.}
